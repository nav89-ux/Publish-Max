...
export function EditProfileModal({ profile }: { profile: Profile }) {
  ...
  return (
    <>
      <button className="profile-edit-button" onClick={() => dialog.current?.showModal()} type="button"><UserIcon /> Edit details</button>
      <dialog className="profile-dialog" ref={dialog}>
        <div className="dialog-heading">
          <div><p className="eyebrow">Public identity</p><h2>Edit details</h2></div>
          <button aria-label="Close profile editor" className="dialog-close" onClick={() => dialog.current?.close()} type="button"><CloseIcon /></button>
        </div>
